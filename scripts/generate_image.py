...
Vertex AI (via environment variables):
    export GOOGLE_GENAI_USE_VERTEXAI=1
    export GOOGLE_CLOUD_PROJECT=your-project-id
    export GOOGLE_CLOUD_LOCATION=global
    export GOOGLE_APPLICATION_CREDENTIALS=~/.config/google-vertex-ai/credentials.json
...
def _is_vertex_mode() -> bool:
    return os.environ.get("GOOGLE_GENAI_USE_VERTEXAI", "0").lower() in ("1", "true", "yes")
...
def main():
    ...
    if not use_vertex:
        ...
        if not api_key:
            ...
            print("  3. Set GOOGLE_GENAI_USE_VERTEXAI=1 for Vertex AI mode", file=sys.stderr)
            sys.exit(1)
    ...
    # Initialise client -- SDK reads GOOGLE_GENAI_USE_VERTEXAI, GOOGLE_CLOUD_PROJECT,
    # GOOGLE_CLOUD_LOCATION, and GOOGLE_APPLICATION_CREDENTIALS automatically.
    if use_vertex:
        client = genai.Client()
